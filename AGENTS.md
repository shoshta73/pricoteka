...
- API migrations live in `apps/api/drizzle/` and are generated from `apps/api/src/db/schema.ts` by Drizzle Kit.
...
- Do not hand-edit Drizzle migration snapshots in `apps/api/drizzle/meta/`; regenerate migrations from schema changes.
...
- `erasableSyntaxOnly`, `noUnusedLocals`, and `noUnusedParameters` are enabled in checked frontend/core configs; unused code fails builds/typechecks.
