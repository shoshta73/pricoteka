...
- pnpm monorepo with the Vite React app package in `app/` as `@pricoteka/app`.
- App entrypoint is `app/src/main.tsx`; TanStack Router uses `createHashHistory()`, so client URLs are hash-based.
- Routes are TanStack Router file routes in `app/src/routes/`; `app/src/routeTree.gen.ts` is generated and should not be edited.
- Static files in `app/public/` are served from the app web root, e.g. reference `app/public/icons.svg` as `/icons.svg`.

## Commands

- Use pnpm workspaces; `pnpm-lock.yaml` is the committed lockfile at the repo root.
...
- App package scripts can be run directly with `pnpm --filter @pricoteka/app <script>`.
- `pnpm app-dev` runs app `prepare:checks` before starting Vite.
- `pnpm app-build` runs app `prepare:checks && tsc -b && vite build`; this is the main verification command.
...
- Edit translations in `app/src/locales/<locale>/translation.json`; i18n uses i18next with typed keys from the English translation file.
