...
- Single-package Vite React app; there is no `pnpm-workspace.yaml` or package boundary to account for.
- App entrypoint is `src/main.tsx`; it creates a TanStack Router with `createHashHistory()`, so client URLs are hash-based.
- Routes are TanStack Router file routes under `src/routes/`; add/change route files there, not in generated code.
- `src/routeTree.gen.ts` is generated by `@tanstack/router-plugin`; do not edit it directly.
...
- `pnpm build` runs `tsc -b && vite build`; this is the main typecheck/build verification.
- `pnpm lint` runs oxlint; `pnpm lint:fix` applies fixes.
- `pnpm format` checks oxfmt; `pnpm format:fix` rewrites files.
...
- `.editorconfig` sets 2-space indentation, LF endings, final newline, trimmed trailing whitespace, and 120-character lines.
